...
import { type SelectedAsset } from '../../queryEditor/iotSiteWiseQueryEditor/modeledDataStreamQueryEditor/types';
...
export class listModeledDataStreamsRequestWithCompositeModels {
  ...
  #formatDataStreams({
    ...
    function isNonNullable<T>(item: T | undefined): item is T {
      return item != null;
    }

    const nonNullableProperties = allProperties.filter(isNonNullable);
